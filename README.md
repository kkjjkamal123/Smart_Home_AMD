...
```
Laptop: Open dashboard.html in Chrome — fullscreen (F11)
Phone:  Open Android app
Pi terminal: Have subscriber.py running showing live prints
Thonny: Show ESP MQTT publish loop running on one board
```

### Script (3 minutes)

**[0:00 – 0:20] Open shot**
> "This is Entity Neural — an intelligent energy monitoring system. Built on ESP32-C6 microcontrollers, a Raspberry Pi 5 hub, and a Random Forest model trained on AMD hardware."
...
**[0:50 – 1:20] Wait for or trigger the surge**
> "Watch the crest factor — when the motor starts, it spikes. That's the startup surge. Our model recognises this signature. Crest factor jumps from 1.4 to over 3.0."

Point to: Crest chip, Surge chip, waveform spike

**[1:20 – 2:00] The load shed moment**
> "Now the system detects total load exceeding the 3A threshold. Room 2 is classified as lower priority — the relay fires, GPIO27 goes LOW, and the motor physically stops."

Show: Card going red, shed alert, relay clicking, motor stopping

**[2:00 – 2:30] AMD benchmark**
> "The model was trained on AMD hardware. Inference on AMD Ryzen AI takes 0.05 milliseconds — 20 times faster than the Pi. In production, we'd deploy this on an AMD edge device for sub-millisecond response."

Point to: AMD benchmark section

**[2:30 – 3:00] Close**
> "Everything runs locally. No cloud. No subscription. Total hardware cost under $20 per room. That's Entity Neural."
